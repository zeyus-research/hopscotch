...
def loadOSIM(file):
    ...
    values = map(lambda x: np.array(x.find('location').text.strip().split(' '), dtype=np.float64)*multiplier, markers)
...
def loadTRC(file):
    ...
    # Read data
    data["Data"] = np.empty(shape=(data["NumMarkers"], data["NumFrames"], 3), dtype=np.float64)
    data["Timestamps"] = np.empty(shape=(data["OrigNumFrames"]), dtype=np.float64)
    for row in reader:
